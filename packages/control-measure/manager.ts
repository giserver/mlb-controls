                ...
                polygon: {
                    measureLineStringOptions: {
                        ...
                        format: (len, i, end, center) => {
                            ...
                            const val = len.toFixed(this.precisions.get(u)) + ` ${this.lang[u]}`;
                            return end ? `${this.lang.ending}: ${val}` : val;
                        }
                    },
                    ...
                    format: area => {
                        ...
                        if (u === 'M2KM2')
                            u = area > 1000000 ? 'KM2' : 'M2';

                        area = units.convertArea(area, 'M2', u);

                        const val = area.toFixed(this.precisions.get(u)) + ` ${this.lang[u]}`;
                        return val;
                    }
                },
                lineString: {
